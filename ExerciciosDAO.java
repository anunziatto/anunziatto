...
public class ExerciciosDAO {
    ...
    public void cadastrarExercicios(ExerciciosDTO objexerciciosdto) {
        String sql = "insert into exercicios ( nome, modalidade,"
                + " tipo, numseries, numrepeticoes,"
                + " duracaototal, tempodedescanso,"
                + " graudeintensidade, descricao) values (?,?,?,?,?,?,?,?,?)";
    ...
    public void excluirExercicios(ExerciciosDTO objexerciciosdto) {
        ...
        int idExercicios = objexerciciosdto.getId_exercicios();

        if (idExercicios == 0) {
            JOptionPane.showMessageDialog(null, "ID do exercício inválido");
            return;
        }
    ...
    public ArrayList<ExerciciosDTO> PesquisarExercicios() {
        ...
        try {
            ...
            //Enquanto estive linha ele busca na lista por isso o rs        
            while (rs.next()) {
